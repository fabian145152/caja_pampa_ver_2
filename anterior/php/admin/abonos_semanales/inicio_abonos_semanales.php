    ...
    <script>
        function deleteProduct(cod_titular) {
            ...
            bootbox.confirm("Desea Eliminar?" + cod_titular, function(result) {
                ...
                if (result) {
                    window.location = "delete_abonos_semanales.php?q=" + cod_titular;
                }
        ...
        function updateProduct(cod_titular) {
            window.location = "edit_abonos_semanales.php?q=" + cod_titular;
        }
    </script>
    <style>
        #Power-Contenedor {
...
<body>
    <?php
    $sql_abonos = "SELECT * FROM abono_semanal";
    $result = $con->query($sql_abonos);

    ?>
    <h1 class="text-center">IMPORTES SEMANALES</h1>
